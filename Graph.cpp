...
class graph
{
    ...
    graph()
    {
        ...
        node* p=NULL;
        node* c=NULL;
        cout<<"Do you want a one directional flight path (1,directed graph) or a bi-directional flight path (0,undirected graph)? :- ";
        cin>>direction;
        cout<<"Enter the number of cities"<<endl;
        cin>>v;
        cout<<"Enter the names of the "<<v<<" cities"<<endl;
        for(i=0;i<v;i++)
        {
            cin>>adjlist[i].city;
        }
        cout<<"Enter adjacent cities of each city"<<endl;
        for(int i=0;i<v;i++)
        {
            p=&adjlist[i];
            cout<<"Enter number of cities adjacent to "<<adjlist[i].city<<" :- ";
            cin>>adjno;
...
int main()
{
    ...
    while(ch=='y')
    {
    	...
    	cout<<"Enter 7 to display graph in adjacency list representation"<<endl; 
    	cin>>choice;
    	switch(choice)
    	{
